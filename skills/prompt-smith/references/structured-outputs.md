...
- [개요](#개요)
- [방법 비교](#방법-비교)
- [방법 1: Format 지시 + 예시](#방법-1-format-지시--예시)
- [방법 2: Prefill 패턴](#방법-2-prefill-패턴)
- [방법 3: XML 구조 강제](#방법-3-xml-구조-강제)
- [방법 4: Tool Use 강제](#방법-4-tool-use-강제)
- [스키마 정의 모범 사례](#스키마-정의-모범-사례)
- [입력-스키마 불일치 처리](#입력-스키마-불일치-처리)
- [검증 체크리스트](#검증-체크리스트)
- [에러 복구 전략](#에러-복구-전략)
- [프롬프트 템플릿](#프롬프트-템플릿)
- [8-Point Quality Check 연계](#8-point-quality-check-연계)
- [관련 참조](#관련-참조)
...
> **주의**: JSON Mode는 **유효한 JSON**만 보장하며, 스키마 준수는 별도 검증이 필요합니다.
...
### 원칙
- 입력에 없는 값은 **추측 금지** (hallucination 방지)
- 필수 필드가 누락될 수 있으면 **스키마에서 nullable/optional 설계**
- 실패 케이스는 **별도 에러 스키마**로 명시하는 것을 권장

### 권장 패턴
1. **오류 채널 포함**: `status`, `errors`, `warnings` 필드로 실패 사유 명시
2. **누락 필드 처리**: 스키마가 허용하는 경우 `null` 또는 빈 배열로 채움
3. **부분 성공 허용**: 부분 결과 + 경고 리스트를 함께 반환
...
### JSON Mode 주의사항 (API)
- JSON Mode는 **유효한 JSON**만 보장하며, **스키마 준수는 보장하지 않습니다**.
- 반드시 **스키마 + 예시 + 금지 규칙**을 함께 제공하고 **서버 측 검증**을 수행하세요.
- 엄격한 구조가 필요하면 **Tool Use** 또는 **Structured Outputs** 방식을 우선 고려합니다.
