# 지연시간 최적화 가이드

Claude API 응답 지연시간을 줄이기 위한 전략과 기법입니다.

> **출처**: Anthropic 공식 문서 - Reduce Latency

---

## 지연시간 측정

### 주요 지표

| 지표 | 설명 | 중요도 |
|------|------|--------|
| **Baseline Latency** | 전체 응답 시간 | 총 사용자 경험 |
| **TTFT** | Time To First Token - 첫 토큰까지 시간 | 체감 응답성 |
| **TPS** | Tokens Per Second - 초당 토큰 생성 | 긴 응답 시 중요 |

### 측정 방법

```python
import time

start = time.time()
response = client.messages.create(...)
ttft = time.time() - start  # 스트리밍 시 첫 청크까지

total_time = time.time() - start
tokens = response.usage.output_tokens
tps = tokens / total_time
```

---

## 최적화 전략 (5가지)

### 1. 모델 선택

| 모델 | 속도 | 품질 | 적합 용도 |
|------|------|------|----------|
| **Haiku** | 🟢 가장 빠름 | 기본 | 간단한 분류, 추출, 짧은 응답 |
| **Sonnet** | 🟡 중간 | 우수 | 복잡한 분석, 코드 생성 |
| **Opus** | 🔴 가장 느림 | 최고 | 복잡한 추론, 창의적 작업 |

**권장**: 작업 요구사항에 맞는 가장 작은 모델 선택

```python
# 간단한 분류 → Haiku
client.messages.create(
    model="claude-3-5-haiku-20241022",
    ...
)

# 복잡한 코드 리뷰 → Sonnet
client.messages.create(
    model="claude-sonnet-4-5-20250514",
    ...
)
```

### 2. 토큰 최적화

#### 입력 토큰 줄이기

```markdown
# ❌ 장황한 프롬프트
You are an incredibly helpful and knowledgeable assistant.
I would really appreciate it if you could help me with the following task.
Please take your time and provide a comprehensive answer.
Now, here is my question: What is 2+2?

# ✅ 간결한 프롬프트
What is 2+2?
```

#### 출력 토큰 줄이기

```markdown
# ❌ 긴 응답 유도
Explain in detail with examples...

# ✅ 짧은 응답 요청
Answer in one sentence.
Limit: 50 words.
Format: [answer only, no explanation]
```

### 3. max_tokens 설정

예상 응답 길이에 맞게 `max_tokens` 설정

```python
# 짧은 응답 예상 시
response = client.messages.create(
    max_tokens=100,  # 짧은 제한
    ...
)

# 긴 응답 필요 시
response = client.messages.create(
    max_tokens=2000,  # 충분한 여유
    ...
)
```

**주의**: `max_tokens`이 너무 낮으면 응답이 잘릴 수 있음

### 4. temperature 조정

| temperature | 효과 | 속도 영향 |
|-------------|------|----------|
| 0.0-0.3 | 결정적, 집중적 | 빠름 |
| 0.5-0.7 | 균형 | 중간 |
| 0.8-1.0 | 창의적, 다양 | 느림 |

```python
# 정확한 추출/분류 → 낮은 temperature
response = client.messages.create(
    temperature=0.1,
    ...
)

# 창의적 글쓰기 → 높은 temperature (속도 희생)
response = client.messages.create(
    temperature=0.8,
    ...
)
```

### 5. 스트리밍 활용

TTFT를 줄여 체감 응답성 향상

```python
# 스트리밍 응답
with client.messages.stream(
    model="claude-sonnet-4-5-20250514",
    messages=[{"role": "user", "content": "..."}],
) as stream:
    for text in stream.text_stream:
        print(text, end="", flush=True)
```

**장점**:
- 첫 토큰이 빠르게 표시
- 사용자가 응답 생성 중임을 인지
- UI 반응성 향상

---

## 작업 유형별 최적화

### 분류/추출 (속도 최우선)

```python
response = client.messages.create(
    model="claude-3-5-haiku-20241022",  # 가장 빠른 모델
    max_tokens=50,                       # 짧은 응답
    temperature=0.0,                     # 결정적
    messages=[{
        "role": "user",
        "content": "Classify: [text]\nOutput: category only"
    }]
)
```

### 코드 생성 (품질과 속도 균형)

```python
response = client.messages.create(
    model="claude-sonnet-4-5-20250514",
    max_tokens=1000,
    temperature=0.2,
    messages=[...]
)
```

### 대화형 응답 (체감 속도)

```python
# 스트리밍으로 TTFT 최소화
async for chunk in client.messages.stream(...):
    yield chunk
```

---

## 프롬프트 최적화 패턴

### 간결한 지시

```markdown
# ❌ 장황함
I would like you to please analyze the following text and
provide me with a summary that captures the main points.
Please make sure to include all the important details.

# ✅ 간결함
Summarize in 3 bullet points:
[text]
```

### 출력 형식 제한

```markdown
# ❌ 무제한 출력
Explain this concept.

# ✅ 제한된 출력
Explain in one paragraph (max 100 words).
```

### Prefill로 프리앰블 제거

```python
# 불필요한 서론 방지
messages = [
    {"role": "user", "content": "List 5 programming languages."},
    {"role": "assistant", "content": "1."}  # 바로 리스트 시작
]
```

---

## 지연시간 vs 품질 트레이드오프

```
┌─────────────────────────────────────────────────────────────┐
│                 Latency-Quality Trade-off                   │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Fast                                                 Slow  │
│  ────────────────────────────────────────────────────────  │
│                                                             │
│  Haiku        Sonnet                           Opus         │
│  temp=0       temp=0.3-0.5                     temp=0.8+    │
│  max_tokens↓  max_tokens medium                max_tokens↑  │
│  No CoT       Simple CoT                       Full CoT     │
│                                                             │
│  Use for:     Use for:                         Use for:     │
│  - 분류       - 코드 생성                      - 복잡한 추론│
│  - 추출       - 분석                           - 창의적 작업│
│  - 라우팅     - 요약                           - 전략 수립  │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

## 체크리스트

```
┌─ Latency Optimization Checklist ────────────────────────────┐
│                                                             │
│  □ 작업에 맞는 가장 작은 모델 선택                          │
│  □ 프롬프트 간결하게 작성                                   │
│  □ 출력 길이 제한 명시                                      │
│  □ 적절한 max_tokens 설정                                   │
│  □ 작업에 맞는 temperature 설정                             │
│  □ 사용자 대면 시 스트리밍 사용                             │
│  □ 불필요한 CoT 제거 (간단한 작업)                          │
│  □ Prefill로 프리앰블 방지                                  │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

## 주의사항

### 과도한 최적화 경고

```markdown
⚠️ 지연시간 최적화와 품질은 트레이드오프 관계

- 너무 짧은 프롬프트 → 품질 저하
- 너무 낮은 max_tokens → 응답 잘림
- 항상 Haiku → 복잡한 작업 실패

권장: 작업 요구사항에 맞는 최소한의 최적화
```

### Prompt Improver와의 관계

```markdown
Prompt Improver는 더 길고 상세한 프롬프트를 생성합니다.
→ 복잡한 작업의 정확도 향상
→ 단, 지연시간 증가

지연시간이 중요한 경우:
- Prompt Improver 결과를 간소화
- 또는 간단한 프롬프트 직접 사용
```

---

## 관련 참조

- [technique-priority.md](technique-priority.md) - 기법 우선순위
- [claude-4x-best-practices.md](claude-4x-best-practices.md) - Claude 4.x 최적화
- [quality-checklist.md](quality-checklist.md) - 품질 체크리스트
